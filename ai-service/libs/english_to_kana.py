...
class EnglishToKana:
    ...
    def is_unpronounceable(self,word):
        """ヘボン式で発音できない単語かどうかを判定する関数"""
        # 子音文字の正規表現
        consonant = r"[^aiueo]"
        # 母音文字の正規表現
        vowel = r"[aiueo]"
        ...
        # ヘボン式ローマ字のルール違反をチェックする正規表現
        rules= [            
            # 子音でも母音でもない文字が含まれている
            r"[^a-z]",
            # 子音が2つ以上連続していて、3文字目がsmall_cでも母音でもない（tokkyo「とっきょ」は許容) syaccho「しゃっちょ」は許容
            consonant + "{2,}[^hyaeiou]",
            # 子音+yの後が母音でない
            consonant + small_c + "[^aeiou]",
            # 子音で終わっている
            consonant + "$",
            # small_cで終わっている
            small_c + "$",
            # 先頭で子音が2つ連続している(ただし、2番目がsmall_cの場合は許容)
            "^" + consonant + "[^hyaiueo]",
        ]
    ...
    def convert(self, word,anyway=False):
        word = word.lower()
        
        if word in self.eng_kana_dic:
            return self.eng_kana_dic[word]
        if not anyway and self.is_unpronounceable(word):
            return None
        return jaconv.alphabet2kata(word)
